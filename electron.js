...
var Smart = (function () {
            ...
            global.AppProtocol.on(evtVar.ProtocolMessage, function(obj){
                try{
                    ...
                    if(obj.Func == "ChangeWindowSize"){
                        ...
                        if(obj.Param =="maximize"){
                            console.log('_this.maximizeStatus',_this.maximizeStatus);

                            if(_this.maximizeStatus){
                                global.MainWindow.win.unmaximize();
                                _this.maximizeStatus=!_this.maximizeStatus;

                            }
                            else{

                                global.MainWindow.win.maximize();
                            }
                            // global.MainWindow.win.setSize(1 , 1);
            ...
            if (isCrashed === null || isCrashed === undefined || !isCrashed){
                if (!opts.noShow){//ShowWindow
                    ...
                    if (commands[0] == '--hide') {
                        ...
                        _this.ReadAPEXConfigDB().then(function (data) {
                            if (data == true) 
                            {
                                global.MainWindow.win.hide();
                            } 
    ...
    Smart.prototype.registerListeners = function () {
        app.on('will-quit', function () {
            ...
            globalShortcut.unregister('ctrl+shift+t'); 
    ...
    Smart.prototype.installTray = function (labs) { 
        ...
        global.TrayIcon.setTitle("MK Series");
        global.TrayIcon.setToolTip("MK Series");
    ...
    Smart.prototype.ReadAPEXConfigDB = function () 
    {
        var nedbObj = require('./backend/dbapi/AppDB').getInstance();

        return new Promise(function (resolve) {
            nedbObj.getAppSetting().then((data)=>{  
                if (data[0].autoStart != undefined && data[0].autoStart[1] == true) 
                    resolve(true);
                else
                    resolve(false);     
            })
